    ...
    public class ProjectController : Controller
    {
        ...
        public async Task<IActionResult> ProjectAddEdit(int? projectId)
        {
            ...
            if (projectId != null)
            {
                ...
                StudentProjectManagement.Areas.Faculty.Models.Project projectData = JsonConvert.DeserializeObject<StudentProjectManagement.Areas.Faculty.Models.Project>(responseContent1);
        ...
        #region SaveProject
        [HttpPost]
        public async Task<IActionResult> SaveProject(StudentProjectManagement.Areas.Faculty.Models.Project project)
        {
            setTokenToHeader();
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                return RedirectToAction("AssignMembers", "Project", new { area = "Faculty", projectId = resObj, edit = project.ProjectId == 0 ? 0 : 1 });
            }
            else
            {
        ...
        #region SaveTask
        public async Task<IActionResult> SaveTask(StudentProjectManagement.Areas.Faculty.Models.Task task)
        {
            setTokenToHeader();
            var userId = HttpContext.Session.GetString("UserId");
            ...
            TempData["Error"] = task.TaskId == 0 ? "Task Not Inserted. Please Try Again" : "Task Not Updated. Please Try Again";
            return RedirectToAction("ProjectDetails", new { projectId = task.ProjectId });
        }
        #endregion
        ...
        public async Task<IActionResult> ProjectEvaluationDelete(int ProjectEvaluationId, int projectId)
        {
            setTokenToHeader();
            var response = await _httpClient.DeleteAsync($"{_configuration["api:url"]}/Evaluation/{ProjectEvaluationId}");
            if (response.IsSuccessStatusCode)
            {
        ...
        #region TaskDelete
        public async Task<IActionResult> TaskDelete(int taskId, int projectId)
        {
            setTokenToHeader();
            var response = await _httpClient.DeleteAsync($"{_configuration["api:url"]}/Task/{taskId}");
            if (response.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> StudentEvaluationDelete(int StudentEvaluationId, int projectId)
        {
            setTokenToHeader();
            var response = await _httpClient.DeleteAsync($"{_configuration["api:url"]}/StudentEvaluation/{StudentEvaluationId}");
            if (response.IsSuccessStatusCode)
            {
